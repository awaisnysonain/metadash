  ...
  return (
    <div className="space-y-4 animate-fade-in" id="team-screen">
      <div className="flex justify-between items-start gap-4">
        <div>
          <h2 className="text-base font-semibold text-slate-950 dark:text-slate-50">Users</h2>
        ...
        {canManage && (
          <button
            ...
            className="px-3.5 py-2 bg-slate-900 dark:bg-slate-100 hover:bg-slate-800 text-white rounded-xl text-sm font-semibold flex items-center gap-2 transition-colors"
          >
      ...
      {showAddForm && canManage && (
        <form onSubmit={handleSubmit} className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl space-y-4 shadow-sm">
          <div className="flex justify-between items-center">
            <h3 className="font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-2">
              <PlusCircle className="w-5 h-5 text-blue-600" /> New team member
            </h3>
            <button type="button" onClick={() => setShowAddForm(false)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-200 block mb-1.5">Slack user ID</label>
              <input type="text" placeholder="U0APP3SEB46" value={slackUserId} onChange={e => setSlackUserId(e.target.value)}
                className="w-full text-sm px-3 py-2 border border-slate-200 dark:border-slate-800 rounded-lg focus:ring-2 focus:ring-slate-300 focus:border-slate-400" />
              <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">Optional. Sends credentials by Slack DM.</p>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input type="text" required placeholder="temporary password" value={password} onChange={e => setPassword(e.target.value)}
                  className="min-w-0 flex-1 text-sm px-3 py-2 border border-slate-200 dark:border-slate-800 rounded-lg focus:ring-2 focus:ring-slate-300 focus:border-slate-400" />
                <button type="button" onClick={handleGenerateCredentials} className="px-3 py-2 text-xs font-semibold rounded-lg border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800">
                  Generate
                </button>
              </div>
            </div>
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-200 block mb-1.5">Full name *</label>
            ...
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-200 block mb-1.5">Job title</label>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                {(Object.keys(PERMISSION_LABELS) as Permission[]).map(perm => (
                  <label key={perm} className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-colors text-xs ${
                    permissions.includes(perm) ? 'border-slate-400 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800'
                  }`}>
          ...
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setShowAddForm(false)} className="px-4 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl">Cancel</button>
            <button type="submit" disabled={submitting}
              className="px-4 py-2 bg-slate-900 dark:bg-slate-100 text-white text-sm font-semibold rounded-xl hover:bg-slate-800 disabled:opacity-60 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-3">
        {listData.map(member => (
          <div key={member.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 hover:shadow-sm hover:border-slate-300 dark:hover:border-slate-600 transition-all flex flex-col">
            <div className="flex items-center gap-3 mb-3">
              {member.avatarUrl ? (
                <img src={member.avatarUrl} alt={member.name} className="w-12 h-12 rounded-xl object-cover" />
              ) : (
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-sm font-bold text-slate-600 dark:text-slate-300">
                  {member.name.charAt(0)}
                </div>
              )}
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <h3 className="font-semibold text-slate-900 dark:text-slate-100 truncate">{member.name}</h3>
            ...
            <div className="space-y-2 flex-1">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-slate-900 dark:text-slate-100">{member.assignedCount}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> Replied
                </span>
                <span className="font-semibold text-slate-900 dark:text-slate-100">{member.completedCount}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1">
                  <AlertCircle className="w-3.5 h-3.5 text-rose-500" /> Urgent
                </span>
                <span className={`font-semibold ${member.urgentCount > 0 ? 'text-rose-600' : 'text-slate-400 dark:text-slate-500'}`}>
                  {member.urgentCount}
                </span>
              </div>
            </div>
            ...
            <button
              ...
              className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 text-sm text-blue-600 font-medium hover:text-blue-700 flex items-center gap-1 transition-colors"
            >
            ...
            {canManage && 'username' in member && member.username && (
              <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800"
                >
      ...
      {listData.length === 0 && !loadingUsers && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center">
          ...
          <p className="text-base font-medium text-slate-700 dark:text-slate-200">No team members yet</p>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Add your first team member to start assigning comments.</p>
        </div>
      )}
    </div>
  );
